...
public class EmployeeDashboardController {
    ...
    public int getCourseProgress() {
        // Get the total number of training modules and the number of completed modules from the TrainingProgressRepository, then calculate and return the course progress percentage for the logged-in employee.
        long totalModules = trainingProgressRepository.count();
        ...
        // Check if there are no training modules to avoid division by zero, and return 0% progress if that's the case
        return (int) ((completedModules * 100) / totalModules);
        // Calculate the course progress percentage by dividing the number of completed modules by the total number of modules, multiplying by 100, and casting the result to an integer
    }//Later, when we implement login, we'll filter it by employee, exactly like in the Trainer Dashboard

    @GetMapping("/employee-dashboard/feedback")// Endpoint to get the latest performance review for the logged-in employee
    public PerformanceReview getFeedback() {
        // Get the latest performance review for the logged-in employee by querying the PerformanceReviewRepository for all reviews, 
        ...
        // Get the list of all performance reviews by querying the PerformanceReviewRepository for all entries
        if (reviews.isEmpty()) {// Check if there are no performance reviews to avoid an IndexOutOfBoundsException, and return null if that's the case
            return null;
        }

    return reviews.get(reviews.size() - 1);
    // Return the latest performance review by getting the last entry in the list of reviews, which is assumed to be sorted by review date 
    // in ascending order
